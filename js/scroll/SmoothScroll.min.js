...
// Detect if user is using Safari/iOS browser
const isSafari = /^((?!chrome|android).)*safari/i.test(navigator.userAgent);
const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;

console.log({ isSafari, isIOS });

// Disable smooth-scroll module if user is using Safari/iOS browser
Date.now =
  Date.now ||
  function () {
...
var SmoothScroll = (function () {
  function SmoothScroll(options) {
    ...
    (this.endThreshold = 0.05),
      ...
      (this._update = function (currentTime) {
        void 0 === currentTime && (currentTime = performance.now());
        var elapsedMS = currentTime - _this.lastTime;
        elapsedMS > _this.maxElapsedMS && (elapsedMS = _this.maxElapsedMS);
        var deltaTime = elapsedMS * _this.targetFPMS,
          dt = 1 - Math.pow(1 - _this.scrollEase, deltaTime),
          resized = _this.resizeRequest > 0,
        ...
        if (resized) {
          ...
          (document.body.style.height = height + "px"),
            ...
            (_this.maxDistance = 2 * _this.viewHeight),
        ...
        (_this.endScroll = scrollY),
          ...
          (Math.abs(scrollY - _this.currentScroll) < _this.endThreshold ||
            resized) &&
            ((_this.currentScroll = scrollY), (_this.scrollRequest = 0));
        ...
        for (var i = 0; i < _this.scrollItems.length; i++) {
          var item = _this.scrollItems[i],
            distance,
            offsetRatio = (scrollOrigin - item.top) / _this.maxDistance;
          (item.endOffset = Math.round(
            ...
            Math.abs(item.endOffset - item.currentOffset < _this.endThreshold)
              ...
              : (item.currentOffset +=
                  (item.endOffset - item.currentOffset) * dt),
      ...
      (this.scrollEase = null != options.scrollEase ? options.scrollEase : 0.1),
      (this.maxOffset = null != options.maxOffset ? options.maxOffset : 500),
      this.addItems(),
  ...
  return (
    (SmoothScroll.prototype.addItems = function () {
      this.scrollItems = [];
      for (
        var elements = document.querySelectorAll("*[data-depth]"), i = 0;
        i < elements.length;
        i++
      ) {
