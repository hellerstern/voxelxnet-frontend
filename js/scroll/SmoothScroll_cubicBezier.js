class SmoothScroll {
  constructor(
    _containerSelector,
    _params = { duration: 500, timingFunction: "easeOutQuart" }
  ) {
    // Init DOM elements
    this.$ = {
  ...
  _initStyle() {
    ...
    this.$.smoothcontainer.style.height = `100vh`;
    ...
    const addTransition = () => {
      ...
      const regex = /\s?([,])\s?/;
      const splitTransform = getComputedStyle(
        this.$.containerBody
      ).transform.split(regex);
      const currentTranslateY = parseInt(
        splitTransform[splitTransform.length - 1]
      );
...
new SmoothScroll(".smoothcontainer");
